...
//            boolean b = m.find();
    ...
    *           If no error is found here, it returns true, or else it returns false*/
    ...
    {
        String duplicateRegex = "((.*)(<\\/(\\w+)>)\\s*(<(\\w+)>.+?<\\/\\6>)*\\s*(<\\4>.*<\\/\\4>))";
        String openTagsRegex = "(<(\\w+)>)";
        String closeTagsRegex = "(<\\/(\\w+)?>)";
        Pattern pat = Pattern.compile(duplicateRegex);
        Matcher m = pat.matcher(tagList);
        ...
        if(m.find()){
            ...
            int i=0;
            for(Map map : fieldAttribs){
                i++;
                if(map.get("name").equals(match)){
                    System.out.println(error + "there is a duplicate tag in record #" + i + " : "+ duplicate);
                    return false;
                }
            }
        }
