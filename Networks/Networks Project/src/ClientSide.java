...
public class ClientSide extends Thread{
    ...
    //JB start() no longer needed, handled in userGo()
	  /*public void start(){  				//called by Client object  
		  ...
		  //for (int i = 0; i < numberOfThreads; i++){*/	
			//  this.run();				//thread.start() calls ClientSide.run()
		  //} 
			  /*float totalTime = 0;
...
//-----------------------------------------------------------------------
    ...
    public static void userGo(String cmd, int count) {
        ...
        CyclicBarrier barrier = new CyclicBarrier(count); //JA 01:30a.m. 03/04/2016 - added + 1 because the last .await() should unblock the barrier, meaning we need one more than the count
        ClientSide.barrier = barrier; //JA 01:30a.m. 03/04/2016 - barrier can now be used elsewhere via ClientSide.barrier
        //clientList.ensureCapacity(count);	//JB -makes sure the ArrayList can hold at least 'count' number of Client objects
        ...
        //theClient.barrier = barrier; //JA 01:30a.m. 03/04/2016 - This should happen after start(), need to start them all and have them ready to go so than when the barrier is down, all of the threads go at once
        clientList.add(theClient);	// add created object to the array list
    	}
    	...
    	//JA 01:30a.m. 03/04/2016 - added try catch- this should be the last .await() which should allow all threads to run parallel
    	/*try {
			ClientSide.barrier.await();
    ...
    public void run() {
    	//JA 01:30a.m. 03/04/2016 - This ClientSide.await() should make this thread wait for the last await method to be called and then it will execute.
    	...
    	//while (clientList.get(i).isAlive == true){
            try {
            	...
            	//if(clientCount > 1){
            	/*barrier.await();*/ //JA 01:30a.m. 03/04/2016 - Should already be awaiting
            	//}
                if(clientCount > 1){
                	try {
                		this.join();
            ...
            /*catch (InterruptedException | BrokenBarrierException e) {
            	...
            	System.exit(1);
            }//end try, catch*/ // JA 01:30a.m. 03/04/2016 - commented out for testing because of removed barrier await statement
           // i++;
    	//}// end while
    	//destroy();
        }//end run()
